...
class SingleWireVirtualizer(Virtualizer):
    ...
    def __init__(self, circuit: QuantumCircuit) -> None:
        ...
        self._Oreg = self._circuit.find_bit(Oqubit).registers[0]
        self._rreg = self._circuit.find_bit(rqubit).registers[0]

    def instantiate(self) -> dict[Fragment, list[Argument]]:
        O_args = [
            {"vswap_0_0": QuantumCircuit(1)},
            {"vswap_0_0": _x_circuit()},
            {"vswap_0_0": _y_circuit()},
        ]
        r_args = [
            {"vswap_0_1": QuantumCircuit(1)},
            {"vswap_0_1": _one_circuit()},
            {"vswap_0_1": _plus_circuit()},
            {"vswap_0_0": _i_circuit()},
        ]
...
def _insert_placeholders_for_vswaps(circuit: QuantumCircuit) -> QuantumCircuit:
    ...
    conf_reg: ClassicalRegister = ClassicalRegister(num_vswaps, "conf")
    vswap_ctr = 0
