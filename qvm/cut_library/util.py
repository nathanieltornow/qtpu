...
from qiskit.circuit import Qubit, QuantumCircuit, Barrier, QuantumRegister
...
def circuit_to_qcg(circuit: QuantumCircuit) -> nx.Graph:
    ...
    for cinstr in circuit.data:
        op, qubits = cinstr.name, cinstr.qubits
...
def decompose_qubits(
    ...
    for cinstr in circuit.data:
        op, qubits, clbits = cinstr.name, cinstr.qubits, cinstr.clbits
        if _in_multiple_fragments(set(qubits)):
