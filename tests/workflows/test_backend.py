...
def sample_circuit_qtpu_backend(
    circuit: QuantumCircuit, num_shots: int
) -> dict[str, int]:
    """
    ...
    evaluator = BackendEvaluator(AerSimulator())
...
def run_comparison(circuit: QuantumCircuit, num_shots: int) -> dict[str, int]:
    """
    Runs the circuit using Qiskit's Backend and returns the counts.
    """
    counts = AerSimulator().run(circuit, shots=num_shots).result().get_counts()
    return counts

@pytest.mark.parametrize("execution_number", range(3))
def test_backend_integration(execution_number):
    """
    ...
    qtpu_counts = sample_circuit_qtpu_backend(circuit, num_shots)
