...
RunCircFuncType = Callable[
    [QuantumCircuit, Iterable[dict[Parameter, float]], int, Any], NDArray
]
...
def sample_virtual_circuit(
    ...
    shots: int = 100000,
    **kwargs: Any,
) -> dict[Fragment, NDArray]:
    if run_circ_func is None:
        run_circ_func = simulate_circuit_expval
...
def simulate_circuit_distr(
    ...
    **kwargs: Any,
    ...
    def _run_circ(circuit: QuantumCircuit, shots: int) -> float:
        ...
        try:
            counts = sim.run(circuit, shots=shots).result().get_counts()
            return prepare_quasidist(
                QuasiDistr.from_counts(counts), num_bits=len(circuit.cregs[0])
            )
        except Exception:
            print("Error running circuit, returning 1.0")
            return QuasiDistr({})
