...
def generate_summation_terms(subcircuits, complete_path_map, num_cuts):
    ...
    """
    print("Generating summation terms...")
    summation_terms = []
    subcircuit_entries = {
    ...
    pprint(complete_path_map)
    pprint(summation_term)
...
def build(summation_terms, subcircuit_entry_probs, num_cuts, counter, verbose):
    ...
    for subcircuit_order in itertools.permutations(range(len(subcircuit_entry_probs))):
        overhead = get_naive_overhead(
            subcircuit_order=subcircuit_order, num_cuts=num_cuts, counter=counter
        )
        smart_order = subcircuit_order
        print("SMARTORDER", smart_order)
        if overhead["additions"] + overhead["multiplications"] < min_overhead:

            min_overhead = overhead["additions"] + overhead["multiplications"]
            if verbose:
                print(
                    "subcircuit_order {}. overhead = {}.".format(
                        subcircuit_order, overhead
                    )
                )

    reconstructed_prob = naive_compute(
        (0, 1), summation_terms, subcircuit_entry_probs
    )
